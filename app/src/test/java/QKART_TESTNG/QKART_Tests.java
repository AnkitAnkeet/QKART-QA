...
public class QKART_Tests {
    ...
    public void TestCase01(String username, String password) throws InterruptedException {
        ...
        Assert.assertTrue( status,"Failed to login with new user");
        ...
        Assert.assertTrue(status, "Logout has been done");
        logStatus("End TestCase", "Test Case 1: Verify user Registration : ", status ? "PASS" : "FAIL");
     ...
     */
    @Test(groups = {"Sanity_test"}, description = "verifying Re-Registration happens Successfully", priority = 2)
    public void TestCase02() throws InterruptedException {
        ...
        Assert.assertFalse(status, "Registration is happening with existing username");
     ...
     */
    @Test(groups = {"Sanity_test"}, description = "Verifying functionality of search text box", priority = 3)
    public void TestCase03() throws InterruptedException {
        ...
        Assert.assertTrue(status,"uanble to find the given product");
        // if (!status) {
        ...
        Assert.assertFalse(status, "Invalid Product");
        // if (status) {
    ...
    public void TestCase04() throws InterruptedException {
        ...
        Assert.assertTrue(status, "Product has not been found");
        List<WebElement> searchResults = homePage.getSearchResults();
        ...
        for (WebElement webElement : searchResults) {
            ...
            if (result.verifySizeChartExists()) {
                ...
                Assert.assertTrue(status, "Size dropdown exists");
                ...
                if (result.openSizechart()) {
                    ...
                    Assert.assertTrue(status, "Size chart is closed");
    ...
    public void TestCase05(String Yonex, String Tan, String Address ) throws InterruptedException {
        ...
        // Register a new user
        status = registration.registerUser("testUser", "abc@123", true);
        Assert.assertTrue(status,"Registration is Unsuccessful");
        if (!status) {
        ...
        status = homePage.searchForProduct("YONEX");
        Assert.assertTrue(status,"Product is found");
        ...
        Assert.assertTrue(status,  "Current url does not end with /thanks");
    ...
    @Parameters({"Stylecon","quantity","Address_1"})
    public void TestCase07(String Stylecon, String quantity, String Address_1) throws InterruptedException {
        Boolean status;
        ...
        registration.navigateToRegisterPage();
        status = registration.registerUser("testUser", "abc@123", true);
        Assert.assertTrue(status,"User cant be registered");
        // if (!status) {
        ...
        Assert.assertTrue(status,"Unable to find Stylecon");
        ...
        Assert.assertTrue(status,"Checkout failed");
    ...
    public void TestCase08() throws InterruptedException {
        ...
        status = homePage.searchForProduct("YONEX");
        Assert.assertTrue(status,"Unable to find YONEx");
        homePage.addProductToCart("YONEX Smash Badminton Racquet");
        ...
        Assert.assertTrue(status,"Actual Cart content is not matching with the expected cart contents");
        driver.close();
    ...
    public void TestCase09() throws InterruptedException {
        ...
        Assert.assertTrue(status,"Unable to verify URL");
        ...
        Assert.assertTrue(status,"Unable to verify text");
        // if (!status) {
    ...
    public void TestCase11() throws InterruptedException {
        ...
        if (!status) {
            logStatus("TestCase 11",
                    ...
                    "FAIL");
            takeScreenshot("Failure", "TestCase11");
        }
        lastGeneratedUserName = registration.lastGeneratedUsername;
        ...
        Assert.assertTrue(status,"Product is not found");
        homePage.addProductToCart("YONEX Smash Badminton Racquet");
        ...
        status = !driver.getCurrentUrl().equals(currentURL);
        Assert.assertTrue(status,"Link is not clickable");
